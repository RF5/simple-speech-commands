...
from convrnn_classifier import ConvRNNConfig, ConvRNNClassifier
...
def train(rank, cfg: TrainConfig):
    ...
    trainset = UtteranceDS(train_df, cfg.sample_rate)
    ...
    if rank == 0:
        validset = UtteranceDS(valid_df, cfg.sample_rate)
                ...
                if steps % cfg.validation_interval == 0 and steps != 0:
                    ...
                    with torch.no_grad():
                        for j, batch in progress_bar(enumerate(validation_loader), total=len(validation_loader), parent=mb):
                            ...
                            flat_logits.append(logits.cpu())
                            flat_lbls.append(lbls.cpu())
